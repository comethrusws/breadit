...
export default async function SubreaditRoute({
  params,
  searchParams,
}: {
  params: { id: string };
        ...
        <Card>
          ...
          <div className="p-4">
            <div className="flex items-center gap-x-3">
              ...
              />
              <Link href={`/br/${data?.name}`} className="font-medium">
                r/{data?.name}
              </Link>
            </div>
            {user?.id === data?.userId ? (
            ...
            <Button asChild className="rounded-full w-full">
              <Link
                href={user?.id ? `/br/${data?.name}/create` : "/api/auth/login"}
              >
                Create Post
              </Link>
            </Button>
